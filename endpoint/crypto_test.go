/*
Copyright 2017 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package endpoint

import (
	"crypto/rand"
	"encoding/base64"
	"io"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestEncrypt(t *testing.T) {
	aesKey := []byte("s%zF`.*'5`9.AhI2!B,.~hmbs^.*TL?;")
	plaintext := "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum."

	t.Run("reject empty nonce", func(t *testing.T) {
		_, err := EncryptText(plaintext, aesKey, "")
		require.EqualError(t, err, "nonce must be provided")
	})

	t.Run("encrypt and decrypt with generated nonce", func(t *testing.T) {
		nonce, err := GenerateNonce()
		require.NoError(t, err)

		encryptedText, err := EncryptText(plaintext, aesKey, nonce)
		require.NoError(t, err)

		decryptedText, _, err := DecryptText(encryptedText, aesKey)
		require.NoError(t, err)
		assert.Equal(t, plaintext, decryptedText, "Original plain text differs from the resulting decrypted text")
	})

	t.Run("decrypt returns error with wrong AES key", func(t *testing.T) {
		nonce, err := GenerateNonce()
		require.NoError(t, err)

		encryptedText, err := EncryptText(plaintext, aesKey, nonce)
		require.NoError(t, err)

		decryptedText, _, err := DecryptText(encryptedText, []byte("s'J!jD`].LC?g&Oa11AgTub,j48ts/96"))
		require.Error(t, err)
		assert.Empty(t, decryptedText, "Data decryption failed, empty string should be as result")
	})

	t.Run("decrypt returns error on unencrypted input", func(t *testing.T) {
		decryptedText, _, err := DecryptText(plaintext, aesKey)
		require.Error(t, err)
		assert.Empty(t, decryptedText, "Data decryption failed, empty string should be as result")
	})

	t.Run("decrypt known encrypted text", func(t *testing.T) {
		encryptedText := ""
		decryptedText, _, err := DecryptText(encryptedText, aesKey)
		require.NoError(t, err)
		assert.Equal(t, plaintext, decryptedText, "Decryption of text didn't result in expected plaintext result.")
	})
}

func TestGenerateNonceSuccess(t *testing.T) {
	nonce, err := GenerateNonce()
	require.NoError(t, err)
	require.NotEmpty(t, nonce)

	// Test nonce length
	decodedNonce, err := base64.StdEncoding.DecodeString(nonce)
	require.NoError(t, err)
	assert.Len(t, decodedNonce, standardGcmNonceSize)
}

func TestGenerateNonceError(t *testing.T) {
	// Save the original rand.Reader
	originalRandReader := rand.Reader
	defer func() { rand.Reader = originalRandReader }()

	// Replace rand.Reader with a faulty reader
	rand.Reader = &faultyReader{}

	nonce, err := GenerateNonce()
	require.Error(t, err)
	assert.Empty(t, nonce)
}

type faultyReader struct{}

func (f *faultyReader) Read(_ []byte) (int, error) {
	return 0, io.ErrUnexpectedEOF
}
